...
export async function POST(request: NextRequest) {
  ...
  // Handle checkout.session.completed for deposit payments (Phase 5)
      ...
      try {
        ...
        await supabase
          ...
          .update({
            ...
            status: booking.status === "contract_signed" ? "payment_pending" : booking.status,
          })
          .eq("id", bookingId)
        ...
        try {
          await fetch(`${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/notifications/send`, {
            ...
            body: JSON.stringify({
              ...
              bookingId,
              amount: parseFloat(session.metadata.baseAmount || "0"),
            }),
          })
        } catch (notifError) {
